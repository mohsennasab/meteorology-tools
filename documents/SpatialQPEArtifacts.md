...
The [NWS Analysis of Record for Calibration (AORC)](https://registry.opendata.aws/noaa-nws-aorc/) dataset provides rainfall estimates at 1-km spatial resolution and 1-hour time steps from 1979 to the present. This period of record supports hydrologic, hydrometeorological, and climate studies. Like all earth observations, gridded QPE contains biases and errors that need review before use. Some artifacts have little effect on rainfall estimates. Others identify data that should be corrected or removed.<br><br>
...
## Gauge Artifacts
**Interpolation Artifacts:** Gauge observations represent conditions at one point. Interpolation spreads each measurement across a spatial grid. Common methods include inverse distance weighting (IDW) and kriging. A sparse gauge network can produce bullseyes or concentric rings around isolated stations. The pattern is strongest when one gauge records heavier rainfall than surrounding gauges. <br><br>
